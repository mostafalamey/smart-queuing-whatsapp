...
export default function TicketCleanupManager({
  className = "",
}: TicketCleanupManagerProps) {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      showError(
        "Error Loading Data",
        "Failed to load ticket cleanup statistics.",
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const handleCleanup = async (hoursOld: number, archive: boolean = true) => {
    try {
      setIsLoading(true);
      const result = await TicketCleanupService.cleanupOldTickets(
        hoursOld,
        archive,
      );
      ...
      showError(
        "Cleanup Failed",
        "There was an error during the cleanup process.",
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const handleAutomatedCleanup = async () => {
    try {
      ...
      showError(
        ...
        "There was an error during automated cleanup.",
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const handleEmergencyCleanup = async () => {
    try {
      ...
      showError(
        ...
        "There was an error during emergency cleanup.",
      );
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div className={`bg-white rounded-lg shadow ${className}`}>
      ...
      <div className="p-6 space-y-6">
            ...
            {totalCounts.archived !== undefined && (
              <div className="mt-3 p-3 bg-purple-50 rounded-lg">
                <span className="text-sm text-purple-800">
                  <strong>{formatNumber(totalCounts.archived)}</strong> tickets
                  archived
                </span>
              </div>
            )}
        ...
        {cleanupNeeded && (
          <div
            className={`p-4 rounded-lg ${cleanupNeeded.needed ? "bg-yellow-50 border border-yellow-200" : "bg-green-50 border border-green-200"}`}
          >
            <div className="flex items-center">
              ...
              <div>
                <div
                  className={`text-sm font-medium ${cleanupNeeded.needed ? "text-yellow-800" : "text-green-800"}`}
                >
                  {cleanupNeeded.needed
                    ? "Cleanup Recommended"
                ...
                <div
                  className={`text-sm ${cleanupNeeded.needed ? "text-yellow-700" : "text-green-700"}`}
                >
                  {cleanupNeeded.reason} (
                  {formatNumber(cleanupNeeded.currentCount)}/
                  {formatNumber(cleanupNeeded.threshold)})
                </div>
              </div>
            </div>
          </div>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {stats.slice(0, 7).map((stat, index) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                        {formatNumber(stat.tickets_created)}
                      </td>
